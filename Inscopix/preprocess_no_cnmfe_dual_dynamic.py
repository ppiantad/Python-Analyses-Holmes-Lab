...
def preprocess(in_path: Path, out_dir: Optional[Path] = None):
    ...
    def _de_interleave_dual_color(
        in_vid: Path,
        out_green_vid: Path,
        out_red_vid: Path
    ):
        print("de-interleaving dual-color...")
        isx.de_interleave_dualcolor(
            [str(in_vid)],
            [str(out_green_vid)],
            [str(out_red_vid)]
        )
    ...
    def _create_paths(in_path: Path, out_path: Path) -> Dict[str, Path]:
        ...
        out["green_deinterleaved_path"] = out_path / f"{actual_name_str}_{isxd_extensions[0]}_green_deinterleaved.isxd"
        out["red_deinterleaved_path"] = out_path / f"{actual_name_str}_{isxd_extensions[0]}_red_downsampled.isxd"
    ...
    _de_interleave_dual_color(in_path, out_paths["green_deinterleaved_path"], out_paths["red_deinterleaved_path"])
    _downsample(out_paths["green_deinterleaved_path"], out_paths["downsample_path"])
