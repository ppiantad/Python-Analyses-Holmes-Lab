...
def preprocess(in_path: Path, out_dir: Optional[Path] = None):
    ...
    def _create_paths(in_path: Path, out_path: Path) -> Dict[str, Path]:
        ...
        out["downsample_path"] = out_path / f"{isxd_extensions[0]}_downsampled.isxd"
        out["spatial_filter_path"] = out_path / f"{isxd_extensions[0]}_spatial_filtered.isxd"
        out["motion_correct_path"] = out_path / f"{isxd_extensions[0]}_motion_corrected.isxd"
        out["tiff_path"] = out_path / f"{isxd_extensions[0]}_motion_corrected.tiff"
